...
# Use the maximum available byte code version to be compatible with the installed JVM
MAX_FD="maximum"
...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    # The pattern is appended to expect $1 to $9.
    for arg do
        case "$arg" in
            -D* | -X* )
                set -- "$@" "$arg"
                ;;
            * )
                MYAPP_HOME=`cygpath --path --windows "$APP_HOME"`
                arg=`echo "$arg" | sed -e "s|\\$APP_HOME|$MYAPP_HOME|g"`
                set -- "$@" "$arg"
                ;;
        esac
        shift
